...
{
  int i, j;
  u16_t t, d, b, text_clicks, data_clicks, ds;
  u32_t data_offset;
  
  data_offset = 512L + (u32_t)sizes[KERN].text_size;
...
{
  long fsck_org;
  u16_t ip, cs, ds, ubuf[SECTOR_SIZE/2], sectors;
                ...
                char *file_name)
{
  ...
  *text_bytes = (u16_t)head[TEXT_POS];
  *data_bytes = (u16_t)head[DATA_POS];
  *bss_bytes  = (u16_t)head[BSS_POS];
}
...
void copy2(int num, char *file_name)
{
  ...
  u16_t text_bytes, data_bytes, bss_bytes, tot_bytes, rest, filler;
  u16_t left_to_read;
...
u16_t get_byte(long offset)
{
  ...
  read_block((u16_t)(offset / SECTOR_SIZE), buff);
  return (buff[(u16_t)(offset % SECTOR_SIZE)] & 0377);
}
...
void put_byte(long offset, int byte_value)
{
  ...
  read_block((u16_t)(offset/SECTOR_SIZE), buff);
  buff[(u16_t)(offset % SECTOR_SIZE)] = byte_value;
  write_block((u16_t)(offset/SECTOR_SIZE), buff);
}
...
void read_block(int blocknr, char *buf)
{
  ...
  fseek(image, (u16_t)blocknr * SECTOR_SIZE, SEEK_SET);
