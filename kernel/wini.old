...
PRIVATE w_dma_setup(struct wini *wn);
PRIVATE int w_transfer(register struct wini *wn);
PRIVATE int win_results(register struct wini *wn);
PRIVATE win_out(int val);
PRIVATE w_reset();
PRIVATE w_wait_int();
PRIVATE win_specify(int drive, struct param *paramp);
PRIVATE check_init();
PRIVATE read_ecc();
PRIVATE hd_wait(register int bits);
PRIVATE com_out(int mode);
PRIVATE init_params();
PRIVATE copy_params(register unsigned char *src, register struct param *dest);
