...
PRIVATE do_write(message *m_ptr);
PRIVATE do_done(message *m_ptr);
PRIVATE do_cancel(message *m_ptr);
PRIVATE reply(int code, int replyee, int process, int status);
PRIVATE pr_error(int status);
PRIVATE print_init();
