...
PRIVATE do_charint(message *m_ptr);
PRIVATE in_char(int line, char ch);
PRIVATE char make_break(char ch);
PRIVATE echo(register struct tty_struct *tp, register char c);
PRIVATE int chuck(register struct tty_struct *tp);
PRIVATE do_read(register struct tty_struct *tp, message *m_ptr);
PRIVATE int rd_chars(register struct tty_struct *ttp);
PRIVATE finish(register struct tty_struct *tp, int code);
PRIVATE do_write(register struct tty_struct *tp, message *m_ptr);
PRIVATE do_ioctl(register struct tty_struct *tp, message *m_ptr);
PRIVATE tty_reply(int code, int replyee, int proc_nr, int status, 
	long extra, long other);
PRIVATE console(register struct tty_struct *tp);
PRIVATE out_char(register struct tty_struct *tp, char c);
PRIVATE scroll_screen(register struct tty_struct *tp, int dir);
PRIVATE flush(register struct tty_struct *tp);
PRIVATE move_to(struct tty_struct *tp, int x, int y);
PRIVATE escape(register struct tty_struct *tp, char x, char y);
PRIVATE set_6845(int reg, int val);
PRIVATE beep(int f);
PRIVATE tty_init();
PRIVATE func_key(char ch);
PRIVATE do_cancel(register struct tty_struct *tp, message *m_ptr);
